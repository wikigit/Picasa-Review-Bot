// DotNetWikiBot Framework v2.7 - bot framework based on Microsoft .NET Framework 2.0 for wiki projects
...
namespace DotNetWikiBot
{
	...
	public class Site
	{
		...
		public void GetPaths()
		{
			...
			Bot.LogEvent(Bot.Msg("Logging in..."));
		...
		public void GetInfo()
		{
            ...
            Bot.LogEvent(Bot.Msg("Site: {0} ({1})"), name, generator);
		...
		private void LogIn(string token)
		{
			...
			string postData = string.Format("lgname={0}&lgpassword={1}&lgdomain={2}&lgtoken={3}", HttpUtility.UrlEncode(userName),
				HttpUtility.UrlEncode(userPass), HttpUtility.UrlEncode(userDomain), HttpUtility.UrlEncode(token == null ? "" : token));
			webReq.Method = "POST";
			webReq.ContentType = Bot.webContentType;
			webReq.UserAgent = Bot.botVer;
			webReq.Proxy.Credentials = CredentialCache.DefaultCredentials;
            ...
            XmlDocument doc = new XmlDocument();
            doc.LoadXml(respStr);
            string result = doc.GetElementsByTagName("login")[0].Attributes["result"].InnerText;
            if (result == "NeedToken")
            {
                token = doc.GetElementsByTagName("login")[0].Attributes["token"].InnerText;
                LogIn(token);
                return;
            }
            else if (result == "Success")
            {
                Bot.LogEvent(Bot.Msg("Logged in as {0}."), userName);
            }
            else
            {
                throw new WikiBotException(
		...
		{
			foreach (DictionaryEntry ns in namespaces) {
                Bot.LogEvent(ns.Key.ToString() + "\t" + ns.Value.ToString().PadRight(20) +
	...
	public class Page
	{
		...
		public void Load()
		{
			...
			try {
                ...
                if (text.Contains("<h1 id=\"firstHeading\" class=\"firstHeading\">Bad title</h1>"))
                {
                    throw new WikiBotException("Tried to Load page with bad title");
                }
			}
			catch (WebException e) {
				string message = Bot.isRunningOnMono ? e.InnerException.Message : e.Message;
            ...
            Bot.LogEvent(Bot.Msg("Page \"{0}\" loaded successfully."), title);
		}
		...
		public void Save(string newText, string comment, bool isMinorEdit)
		{
			...
			if (site.editSessionTokenRE1.IsMatch(respStr)) {
                ...
                Bot.LogEvent(Bot.Msg("Page \"{0}\" saved successfully."), title);
				text = newText;
			}
		}
		...
		public void Watch()
		{
			...
			site.GetPageHTM(res);
			watched = true;
            Bot.LogEvent(Bot.Msg("Page \"{0}\" added to watchlist."), title);
		}
		...
		public void Unwatch()
		{
			...
			site.GetPageHTM(res);
			watched = false;
            Bot.LogEvent(Bot.Msg("Page \"{0}\" was removed from watchlist."), title);
		}
        ...
        private static string HtmlEncodeSpecialChars(string s)
        {
            s = s.Replace("&", "%26");
            s = s.Replace("'", "%27");
            s = s.Replace("\"", "%22");
			...
			string license, string copyStatus, string source)
		{
			...
			if (Path.GetFileNameWithoutExtension(filePathName).Length < 3)
				throw new WikiBotException(string.Format(Bot.Msg("Name of file \"{0}\" must " +
					...
					filePathName));
            Bot.LogEvent(Bot.Msg("Uploading image \"{0}\"..."), title);
            ...
            webReq.Timeout = int.MaxValue; // Give it as long as we can, some uploads are really big
			webReq.Proxy.Credentials = CredentialCache.DefaultCredentials;
			webReq.UseDefaultCredentials = true;
			webReq.Method = "POST";
			...
			for (int errorCounter = 0; errorCounter <= Bot.retryTimes; errorCounter++) {
				try {
                    ...
                    if (message == "The operation has timed out" ||
                        Regex.IsMatch(message, ": \\(50[02349]\\) ")) {		// Remote problem
						if (errorCounter >= Bot.retryTimes)
							throw;
            ...
            // DC: This bit is broken
			if (/*!respStr.Contains(HtmlEncodeSpecialChars(targetName)) ||*/
				...
				text = description;
                Bot.LogEvent(Bot.Msg("Image \"{0}\" uploaded successfully."), title);
#if false
			}
#endif
		}
		...
		public void DownloadImage(string filePathName)
		{
			...
			string fileLink = "";
            // DC: Switch RE1 and RE2 here, as some images (e.g. File:Linor Goralik by Anton Nossik crop.jpg)
            // were matching links to other image description pages with RE1.
			if (fileLinkRE2.IsMatch(src))
				fileLink = fileLinkRE2.Match(src).Groups[1].ToString();
			else if (fileLinkRE1.IsMatch(src))
			...
			Bot.InitWebClient();
            Bot.LogEvent(Bot.Msg("Downloading image \"{0}\"..."), title);
			Bot.wc.DownloadFile(fileLink, filePathName);
            Bot.LogEvent(Bot.Msg("Image \"{0}\" downloaded successfully."), title);
		}
		...
		public void SaveToFile(string filePathName)
		{
            ...
            Bot.LogEvent(Bot.Msg("Text of \"{0}\" page successfully saved in \"{1}\" file."),
				title, filePathName);
		}
		...
		public bool IsDisambig()
		{
            ...
            Bot.LogEvent(Bot.Msg("Initializing disambiguation template tags..."));
		...
		/// array can may recur, indicatinf that file was mentioned several times.</returns>
		...
		public void Delete(string reason)
		{
            ...
            Bot.LogEvent(Bot.Msg("Page \"{0}\" was successfully deleted."), title);
			title = "";
		}
	}
	...
	public class PageList
	{
			...
			int quantity)
		{
            ...
            Bot.LogEvent(Bot.Msg("PageList filled with {0} page titles from " +
		...
		public void FillFromCustomSpecialPage(string pageTitle, int quantity)
		{
			...
			if (quantity <= 0)
				throw new ArgumentOutOfRangeException("quantity",
					Bot.Msg("Quantity must be positive."));
            Bot.LogEvent(Bot.Msg("Getting {0} page titles from \"Special:{1}\" page..."),
				quantity, pageTitle);
			...
			int quantity)
		{
			...
			if (quantity <= 0)
				throw new ArgumentOutOfRangeException("quantity",
					Bot.Msg("Quantity must be positive."));
            Bot.LogEvent(Bot.Msg("Getting {0} page titles from \"{1}\" log..."),
			...
			foreach (Match match in matches)
				pages.Add(new Page(site, HttpUtility.HtmlDecode(match.Groups[1].Value)));
            Bot.LogEvent(Bot.Msg("PageList filled with {0} page titles from \"{1}\" log."),
		...
		public void FillFromCustomBotQueryList(string listType, string queryParams, int quantity)
		{
			...
			if (!string.IsNullOrEmpty(Environment.StackTrace) &&
				!Environment.StackTrace.Contains("FillAllFromCategoryEx"))
					Bot.LogEvent(Bot.Msg("PageList filled with {0} page titles " +
			...
			bool hideLogged, bool hideSelf, int limit, int days)
		{
			...
			Bot.LogEvent(Bot.Msg("Getting {0} page titles from " +
		...
		public void FillAllFromCategory(string categoryName)
		{
			...
			Bot.LogEvent(Bot.Msg("Getting category \"{0}\" contents..."), categoryName);
			//RemoveAll();
			if (Bot.useBotQuery == true && site.botQuery == true) {
		...
		public void FillAllFromCategoryEx(string categoryName)
		{
			...
			if (site.botQueryVersions.ContainsKey("ApiQueryCategoryMembers.php")) {
				...
				Bot.LogEvent(Bot.Msg("Can't get category members using bot interface.\n" +
		...
		public void FillFromPageHistory(string pageTitle, int lastRevisions)
		{
			...
			XmlDocument doc = new XmlDocument();
            doc.XmlResolver = null;
            string text = site.GetPageHTM(res).Replace("&nbsp;", " ").Replace("&ndash;", "-");
            doc.LoadXml(text);
			...
			Bot.LogEvent(Bot.Msg("PageList filled with {0} last revisons of \"{1}\" page..."),
				nl.Count, pageTitle);
		}
		...
		public void FillFromWatchList()
		{
			...
			foreach (Match match in matches)
				pages.Add(new Page(site, HttpUtility.HtmlDecode(match.Groups[1].Value)));
			Bot.LogEvent(Bot.Msg("PageList filled with bot account's watchlist."));
		}
        ...
        const int ResultsPerPageLimit = 500;
		...
		public void FillFromSearchResults(string searchStr, int limit)
		{
            ...
            do
            {
                int numResults = Math.Min(limit, ResultsPerPageLimit);
                string res = site.site + site.indexPath +
                    "index.php?title=Special:Search&fulltext=Search&search=" +
                    HttpUtility.UrlEncode(searchStr) + "&limit=" + numResults.ToString() + "&offset=" + offset;
                string src = site.GetPageHTM(res);
                ...
                foreach (Match match in matches)
                {
                    if (match.Groups[1].Value != "Log out")
                    {
                        pages.Add(new Page(site, HttpUtility.HtmlDecode(match.Groups[1].Value)));
                        addedMatches++;
                    }
			...
			Bot.LogEvent(Bot.Msg("PageList filled with search results."));
		}
		...
		public void FillFromGoogleSearchResults(string searchStr, int limit)
		{
			...
			Bot.LogEvent(Bot.Msg("PageList filled with www.google.com search results."));
		}
		...
		public void LoadEx2()
		{
			...
			Bot.LogEvent(Bot.Msg("Pages download completed."));
		}
		...
		public void SaveTitlesToFile(string filePathName, bool useSquareBrackets)
		{
			...
			Bot.LogEvent(Bot.Msg("Titles in PageList saved to \"{0}\" file."), filePathName);
		}
		...
		{
			Bot.LogEvent(Bot.Msg("Loading {0} pages for XML dump..."), this.pages.Count);
			string res = site.site + site.indexPath +
				"index.php?title=Special:Export&action=submit";
	...
	public class Bot
	{
		...
		public static readonly Version version = new Version("2.7");
        ...
        public static bool ShowEventTraceOnConsole = true;

        public static void LogEvent(string message, params object[] objs)
        {
            if (ShowEventTraceOnConsole)
            {
